...
| 8.1c | SSH transport wiring: `GitSshSessionFactory` supplying the repository's key (or the default) per operation, `known_hosts` with a fingerprint confirmation dialog on first contact and a hard refusal on mismatch, `GitRemoteUrlValidator` accepting `ssh://` and `git@host:path`, remote setup and clone dialogs offering SSH, error mapping (publickey → `AUTH_FAILED`), passphrase prompt for imported keys. | **Fable** | M | Depends on 8.1a and 8.1b; the exact list is "What the next two lanes must implement (b)" in `doc/adr/0002-ssh-on-android.md`. `GitSshSessionFactory` and the `AUTH_FAILED` mapping already exist from 8.1a; the key store, `GitRepoConfig.sshKeyId` and `GitSshKeySelection.resolve` exist from 8.1b |
