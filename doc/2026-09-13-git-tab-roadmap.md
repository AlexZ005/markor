...
| D4 | **Transport: HTTPS + personal access token first.** SSH is a later phase. **Revised 2026-09-14 (task 8.1a): SSH is supported alongside HTTPS**, on JGit's `ssh.jsch` module with the maintained `com.github.mwiede:jsch` fork, RSA/ECDSA keys only (no ed25519, which would need Bouncy Castle at +2.1 MB). HTTPS stays the default the dialogs offer. See `doc/adr/0002-ssh-on-android.md`. | JGit's SSH stack (Apache MINA sshd) is unproven on Android and heavy. HTTPS with a token covers GitHub, GitLab, Gitea/Forgejo, Codeberg. The spike found the JSch route costs only +292 KB / +3,141 methods in the release APK and passes clone/fetch/pull/push on api26 in debug and under R8, so the "heavy" objection does not apply to it; MINA sshd was never needed. |
...
|----|------|-------|------|-------|
| 8.1a | **Pulled forward 2026-09-14 (user request).** SSH spike: which SSH stack works with JGit 5.13.5 on API 26 (`ssh.jsch` + mwiede JSch 2.28.7 first, MINA sshd only as fallback), key types generated in-app (RSA 4096, ed25519 with/without Bouncy Castle), importable key formats, trust-on-first-use `known_hosts`, clone/fetch/pull/push against GitHub with a deploy key, R8 rules, APK delta. ADR `doc/adr/0002-ssh-on-android.md`. | **Fable** | M | ✅ shipped. Verdict: `ssh.jsch` + `com.github.mwiede:jsch:2.28.7`, RSA 4096 and ECDSA, no ed25519 (Bouncy Castle costs +17 % APK), OpenSSH-v1 key storage, TOFU `known_hosts` in `filesDir/git/`, one R8 keep rule. |
| 8.1b | SSH key store and settings UI: keys kept in app-private storage (private key encrypted with a Keystore-wrapped key), a key registry with one **default key**; Settings > Git > identity section gains **"SSH key"** right after "Author name" and "Author e-mail" (generate / import from file / copy or share public key / set default / delete, fingerprint shown); `GitRepoConfig.sshKeyId` for a **per-repository key** chosen in the repository's settings (default = the app default). | Opus | M | ✅ shipped. `GitSshKeyStore` (keys under `filesDir/git/ssh`, private halves AES/GCM-wrapped by an Android Keystore key), `GitSshKey`, `GitSshKeySelection`, `GitRepoConfig.sshKeyId`, Settings > Git > **SSH key** → `SshKeyManagerActivity`, and the per-repository choice in `RemoteSetupDialog`. Generated keys: RSA 4096 / ECDSA nistp256, OpenSSH v1, no passphrase; imported keys are stored byte for byte, so a passphrase-protected file stays protected and 8.1c has to ask for the passphrase. |
| 8.1c | SSH transport wiring: `GitSshSessionFactory` supplying the repository's key (or the default) per operation, `known_hosts` with a fingerprint confirmation dialog on first contact and a hard refusal on mismatch, `GitRemoteUrlValidator` accepting `ssh://` and `git@host:path`, remote setup and clone dialogs offering SSH, error mapping (publickey → `AUTH_FAILED`), passphrase prompt for imported keys. | **Fable** | M | ✅ shipped. `JGitSsh` attaches a fresh `GitSshSessionFactory` per operation (clone, fetch, pull, push, ls-remote) through `setTransportConfigCallback`, never the global `SshSessionFactory.setInstance`; `GitSshAuth` resolves the key, asks for a passphrase and answers the fingerprint question; `GitRemoteUrlPolicy` answers with a `Transport` so the token reaches https only and the key SSH only; `GitSshRemoteTrust` refuses a key to a remote the app did not record; `GitKnownHosts` + Settings > Git > **Known SSH servers** list and forget hosts; `GitResult.HOST_KEY_MISMATCH`. Verified on api26 in the debug and the R8 release APK, clone and push included. |
...
5. ~~Do you want SSH (Phase 8.1) pulled forward? It roughly doubles the risk and size budget.~~ Answered 2026-09-14: yes, pulled forward as 8.1a/8.1b/8.1c. The size budget was not doubled — the SSH stack costs +2.4 % of the release APK (`doc/adr/0002-ssh-on-android.md`).
