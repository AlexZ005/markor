...
<br/>🌿 Git tab (**fork feature**, not in upstream Markor): pick a folder that is a git repository, review and commit your changes, browse the history with diffs, and pull from and push to a remote -- HTTPS with a personal access token, or SSH with a key the app generates or imports. Do not let a file-sync app carry the `.git` folder as well -- pick one of the two.
